...
class Net_SDE(nn.Module):
    ...
    def forward(self, S0, V0, rate,BS_vol, indices, z,z1, MC_samples): 
      ...
      # BS_old = torch.repeat_interleave(S0, MC_samples, dim=0)
        ...
        irand = randrange(1, len(self.timegrid))
        ...
        for i in range(1, len(self.timegrid)):
            ...
            if i != irand:
                ...
                V_new = V_old + self.driftV(inputNN).detach()*h +self.diffusionV(inputNN).detach()*dW + self.diffusionV1(inputNN).detach()*dW1
            else:
...
for i in range(2,3):
    np.save('seeds_used2602.npy', seedsused) 
    np.save('losses2602.npy', losses)
    np.save('losses_val2602.npy', losses_val)
    seedsused[i-1,0],model=train_models(int(seedsused[i-2,0]))
    path = "Neural_SDE2602_"+str(i-1)+".pth"
