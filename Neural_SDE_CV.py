        ...
        for i in range(1, len(self.timegrid)):
          ...
          # Evaluate put (ITM) and call (ITM) option prices 
                ...
                for strike in K_put:
                    ...
                    price_ITM = torch.cat([-K_extended_call+S_old,zeros],1) #Put OTM
                    price = torch.cat([-S_old+K_extended,zeros],1) #Call OTM
                    price = torch.max(price, 1, keepdim=True)[0]*torch.exp(-rate*1*i/n_steps)+S0-strike*torch.exp(-rate*1*i/n_steps)  
                    price_ITM = torch.max(price_ITM, 1, keepdim=True)[0]*torch.exp(-rate*1*i/n_steps)-S0+strike_call*torch.exp(-rate*1*i/n_steps)  
...
for i in range(2,102):
    np.save('seeds_used_43.npy', seedsused) 
    np.save('losses_43.npy', losses)
    np.save('losses_43.npy', losses_val)
    seedsused[i-1,0],model=train_models(int(seedsused[i-2,0]))
    path = "Neural_SDE_43_"+str(i-1)+".pth"
    torch.save(model.state_dict(), path)  
